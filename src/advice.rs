/// The value fed to the platform's `madvise` call.
///
/// rustix v1 does not model the Darwin-specific advice values this crate
/// exposes (`MADV_FREE`, `MADV_FREE_REUSABLE`, `MADV_FREE_REUSE`,
/// `MADV_ZERO_WIRED_PAGES`), so Apple targets carry a raw `c_int`
/// (always `i32` on Apple) and route through `libc::madvise`. Every other
/// unix platform carries a `rustix::mm::Advice` and routes through
/// `rustix::mm::madvise`. Each platform populates exactly one variant.
...
impl UncheckedAdvice {
  pub(crate) fn to_raw(self) -> RawAdvice {
    use rustix::mm::Advice as R;
    RawAdvice::Rustix(match self {
      // On Linux, prefer the MADV_DONTNEED semantics; elsewhere rustix
      // only offers POSIX_MADV_DONTNEED via `Advice::DontNeed`.
      #[cfg(target_os = "linux")]
